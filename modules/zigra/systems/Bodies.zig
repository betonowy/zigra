...
pub fn tickProcessBodies(self: *@This(), ctx_base: *lifetime.ContextBase) !void {
    ...
    while (iterator.next()) |fields| {
        switch (fields.payload.*) {
            ...
            .rigid => |*r| try self.processBodyRigid(&view, &transforms[r.id_transform], r, &meshes[r.id_mesh], delay),
...
fn processBodyRigid(
    ...
    const sink_correction_sum: @Vector(2, f32) = .{ 0, 0 };
    const sink_correction_count: i32 = 0;
    const hit_count: i32 = 0;

    for (mesh.points.constSlice()) |p| {
        try self.processBodyRigidPointCollision(view, t_curr, b_curr, &t_next, &b_next, mesh, p, delay);
        try self.processBodyRigidPointSinking(view, t_curr, b_curr, &t_next, &b_next, mesh, p, delay);
    }

    if (sink_correction_count > 2) {
        b_next.sleeping = true;
    }

    t_next.vel += sink_correction_sum;
    ...
    // if (la.length(t_curr.pos - t_next.pos) < 3e-2 and
    //     hit_dot > 0 and scale_ratio == 0 and @abs(@reduce(.Add, t_next.vel * @Vector(2, f32){ hit_nor[1], -hit_nor[0] })) < 1e-3)
    // {
    //     b_next.sleeping = true;
    //     t_next.vel = .{ 0, 0 };
    // }
...
fn processBodyRigidPointCollision(
    ...
    const msq_result = try systems.World.Marching.intersect(view, pos_curr, pos_next, solidCmp) orelse return;
    ...
    const impulse = brk: {
        ...
        const mag_rebound_normal = -(2 - mesh.bounce_loss) * dot_nor / (1 / mesh.mass + la.dot(hit_nor, cross_2d)) + @as(f32, 0);
        const mag_friction_tangent = -@max(mag_rebound_normal * mesh.bounce_loss, 0);
