...
pub fn init(device: *zvk.Device) !@This() {
    ...
    const sm_vs = try zvk.ShaderModule.init(device, &spv.render_world_vert);
    defer sm_vs.deinit();

    const sm_fs = try zvk.ShaderModule.init(device, &spv.render_world_frag);
    defer sm_fs.deinit();
    ...
    const pipeline = try zvk.Pipeline.initGraphics(device, .{
        ...
        .target_info = .{ .color_attachments = &.{.r16g16b16a16_sfloat} },
    });
